...
window.toggleLedDirect = function() {
    ...
    if (!deviceIP || deviceIP === "Unknown" || deviceIP.startsWith("127.0.0.1")) {
        deviceIP = localStorage.getItem("lastWorkingDeviceIP");
        if (deviceIP && deviceIP !== "Unknown" && deviceIP !== "Offline" && !deviceIP.startsWith("127.")) {
            console.log(`Using stored IP from localStorage: ${deviceIP}`);
        } else {
            console.log("No valid IP found in localStorage");
            deviceIP = "No-IP-Available";
        }
    }
    ...
    // Check if a valid IP is available
    if (deviceIP === "No-IP-Available") {
        console.error("No valid device IP available for LED control");
        
        // Update status display with error
        if (statusElement) {
            statusElement.innerHTML = `<i class="bi bi-exclamation-triangle"></i> Unable to control LED: No device IP available. Connect to a device first.`;
            statusElement.className = "alert alert-warning text-center fs-5 p-3 mb-4";
        }
        return; // Stop execution if no valid IP
    }
    
    // Skip ping checks and go straight to LED toggle for better response
    console.log("Skipping ping check - going straight to LED toggle");
...
function getBestDeviceIP() {
    ...
    // First check if we have an explicit selectedDeviceIP from script.js
    if (window.selectedDeviceIP && window.selectedDeviceIP !== "Unknown" && 
        window.selectedDeviceIP !== "Offline" && !window.selectedDeviceIP.startsWith("127.")) {
        deviceIP = window.selectedDeviceIP;
        console.log(`Using selected device IP from script.js: ${deviceIP}`);
        return deviceIP;
    }
    
    // Next try to get the IP from the device that shows "Online" in the status column
    const onlineRows = document.querySelectorAll("tr:has(td:nth-child(3) .bg-success)");
        ...
        for (const row of onlineRows) {
            ...
            if (ipCell && ipCell.textContent) {
                ...
                if (ip && ip !== "Unknown" && ip !== "Offline" && !ip.startsWith("127.")) {
                    deviceIP = ip;
                    console.log(`Found IP from online device row: ${deviceIP}`);
                    return deviceIP;
                }
            }
        }
    ...
    // Try to get the IP from the selected row (with table-primary class)
    const selectedRow = document.querySelector("tr.table-primary");
    if (selectedRow) {
        const ipCell = selectedRow.querySelector("td:nth-child(4)");
        if (ipCell && ipCell.textContent) {
    ...
    // Check for explicitly connected IP in the connection status text
    const connectionStatusText = document.getElementById("deviceConnectionStatusText");
    if (connectionStatusText) {
        const statusText = connectionStatusText.textContent || connectionStatusText.innerText;
        if (statusText && statusText.includes("Connected to:")) {
            // Extract the IP address with regex
            const ipMatch = statusText.match(/Connected to:.*?(\d+\.\d+\.\d+\.\d+)/);
            if (ipMatch && ipMatch[1]) {
                deviceIP = ipMatch[1];
                console.log(`Found IP in connection status text: ${deviceIP}`);
                return deviceIP;
            }
        }
    }
    
    // If no IP found in selected row, look for any row with a valid IP
    if (!deviceIP) {
        ...
        for (const row of allRows) {
            ...
            if (ipCell && ipCell.textContent) {
                ...
                if (ip && ip !== "Unknown" && ip !== "Offline" && !ip.startsWith("127.")) {
                    deviceIP = ip;
                    console.log(`Found IP in table row: ${deviceIP}`);
                    return deviceIP;
                }
            }
        }
    ...
    // If still no IP, try the device info section
    if (!deviceIP) {
        ...
        if (deviceIpElement && deviceIpElement.textContent) {
            ...
            if (ip && ip !== "Unknown" && ip !== "Offline" && !ip.startsWith("127.")) {
                deviceIP = ip;
                console.log(`Found valid IP in device info display: ${deviceIP}`);
                return deviceIP;
            } else {
    ...
    // Try looking for any IP in the document that matches 192.168.*.*
    if (!deviceIP) {
        ...
        while (textNodes.nextNode()) {
            const text = textNodes.currentNode.textContent;
            // Look for IP pattern
            const ipMatch = text.match(/\b192\.168\.\d+\.\d+\b/);
            if (ipMatch) {
                deviceIP = ipMatch[0];
                console.log(`Found IP pattern in page text: ${deviceIP}`);
                return deviceIP;
            }
        }
    }
    
    // Last resort - check for stored IP in localStorage
    if (!deviceIP) {
        const lastIP = localStorage.getItem("espIP") || localStorage.getItem("lastWorkingDeviceIP");
        if (lastIP && lastIP !== "Unknown" && lastIP !== "Offline" && !lastIP.startsWith("127.")) {
            deviceIP = lastIP;
            console.log(`Using stored IP from localStorage: ${deviceIP}`);
            return deviceIP;
        }
    ...
    console.log(`No valid IP address found, returning Unknown`);
    return "Unknown";
}
...
function directLedToggle(ipAddress) {
    ...
    // Check if a valid IP is available
    if (ipAddress === "No-IP-Available") {
        console.error("No valid device IP available for LED control in fallback");
        ...
        // Update status display with error
        if (statusElement) {
            statusElement.innerHTML = `<i class="bi bi-exclamation-triangle"></i> Unable to control LED: No device IP available.`;
            statusElement.className = "alert alert-warning text-center fs-5 p-3 mb-4";
        }
        return; // Stop execution if no valid IP
    }
...
window.directLedControl = function(turnOn) {
    ...
    if (ip === "No-IP-Available") {
        console.error("Cannot control LED - no valid device IP available");
        alert("Cannot control LED: No device IP available. Please connect to a device first.");
        return;
    }
...
window.testLed = function(state) {
    ...
    const deviceIP = getBestDeviceIP();
    if (deviceIP === "No-IP-Available") {
        console.error("Cannot test LED: No device IP available");
        alert("Cannot connect to device: No IP address available. Please connect to a device first.");
        return;
    }
    
    const url = `http://${deviceIP}/led?state=${apiState}`;
    
    console.log(`Testing LED: Sending API state=${apiState} to make LED visually ${visualState}`);
    ...
    .then(response => {
        if (response.ok) {
            ...
            console.error(`LED test failed: ${response.status}`);
            alert(`Failed to test LED: ${response.status}`);
        }
    })
    .catch(error => {
        console.error(`LED test error: ${error.message}`);
        alert(`Error: ${error.message}`);
    });
};
